---
title: "Renewal/Churn Rates & Predicting Churn"
...
buyer_behavior = read.csv("sum_tickets.csv") # from buyer_behavior.Rmd
```
...
ggplot(churn_rates_by_pack %>% filter(plan_type != "TICKET_SOLD", plan_type != "SINGLE_TICKET_SOLD",
                                      plan_type != "SEASON_TICKET_SOLD", plan_type != "MINI_PLAN_TICKET_SOLD"), 
       aes(x = SEASON_YEAR, y = churn_rate, color = plan_type)) + 
  geom_point() + geom_line() + ggtitle("Churn Rate by Plan") + theme_minimal() +
  scale_x_continuous(breaks = seq(min(churn_rates_by_pack$SEASON_YEAR), max(churn_rates_by_pack$SEASON_YEAR), 
                                  by = 1))

ggplot(churn_rates_by_pack %>% filter(plan_type != "TICKET_SOLD", plan_type != "SINGLE_TICKET_SOLD",
                                      plan_type != "SEASON_TICKET_SOLD", plan_type != "MINI_PLAN_TICKET_SOLD"), 
       aes(x = SEASON_YEAR, y = renewal_rate, color = plan_type)) + 
  geom_point() + geom_line() + ggtitle("Renewal Rate by Plan") + theme_minimal() +
  scale_x_continuous(breaks = seq(min(churn_rates_by_pack$SEASON_YEAR), max(churn_rates_by_pack$SEASON_YEAR), 
                                  by = 1))
```
...
churners_demo2 <- buyer_long[churners_demo, on = .(ACCOUNT_TYPE, SEASON_YEAR), allow.cartesian = TRUE]
```
